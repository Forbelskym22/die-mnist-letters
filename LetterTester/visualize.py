"""
LetterTester Visualizer - GUI vizualizace výsledků testování písmen

Zobrazuje výsledky z JSON souboru vytvořeného LetterTesterem:
- Overall metrics s barevným indikátorem
- Confusion matrix heatmap (interaktivní) pro 26 tříd A-Z
...
class ResultsVisualizer:
    def __init__(self, results_path: Path):
        ...
        self.window = tk.Tk()
        self.window.title("LetterTester - Výsledky testování písmen A-Z")
    ...
    def _create_header(self):
        ...
        title = tk.Label(
            header_frame,
            text="🔤 Výsledky testování modelu písmen A-Z",
            font=("Arial", 20, "bold"),
            bg=BG_COLOR,
            fg=FG_COLOR,
        )
    ...
    def _create_overall_panel(self):
        ...
        num_classes = self.results.get("num_classes", 26)
        ...
        classes_label = tk.Label(
            inner,
            text=f"Třídy: {num_classes} (A-Z)",
            font=("Arial", 14),
            bg=BUTTON_BG,
            fg=FG_COLOR,
        )
        classes_label.pack(side=tk.LEFT, padx=20)

    def _create_confusion_heatmap(self, parent):
        """Vytvoří confusion matrix jako heatmap pro 26 tříd."""
        label = tk.Label(
            parent,
        ...
        # Create figure with dark background - larger for 26 classes
        self.confusion_fig = Figure(figsize=(8, 7), facecolor=BG_COLOR)
        ...
        # Axes - use letters A-Z
        self.confusion_ax.set_xticks(np.arange(num_classes))
        self.confusion_ax.set_yticks(np.arange(num_classes))
        self.confusion_ax.set_xticklabels(LETTERS[:num_classes], color=FG_COLOR, fontsize=8)
        self.confusion_ax.set_yticklabels(LETTERS[:num_classes], color=FG_COLOR, fontsize=8)
        ...
        # Add text annotations (smaller font for 26x26 matrix)
        for i in range(num_classes):
            for j in range(num_classes):
                val = confusion[i, j]
                if val > 0:  # Only show non-zero values
                    text_color = "black" if val > confusion.max() / 2 else "white"
                    fontsize = 6 if val < 100 else 5
                    self.confusion_ax.text(j, i, str(val),
    ...
    def _create_perclass_charts(self, parent):
        """Vytvoří bar charts pro per-class metriky (26 tříd)."""
        label = tk.Label(
            parent,
        ...
        for letter in LETTERS:
            if letter in metrics:
                accuracies.append(metrics[letter]["accuracy"])
                precisions.append(metrics[letter]["precision"])
                recalls.append(metrics[letter]["recall"])
                f1_scores.append(metrics[letter]["f1_score"])
            else:
                accuracies.append(0)
                precisions.append(0)
                recalls.append(0)
        ...
        # Create figure - wider for 26 classes
        self.metrics_fig = Figure(figsize=(8, 4), facecolor=BG_COLOR)
        ...
        self.metrics_ax.set_xticklabels(LETTERS, color=FG_COLOR, fontsize=7)
    ...
    def _create_confusions_list(self, parent):
        ...
        top_confusions = confusions[:8]  # Show top 8 for letters
        ...
        if not top_confusions:
            ...
            for idx, (true_cls, pred_cls, count) in enumerate(top_confusions, 1):
                true_letter = LETTERS[true_cls]
                pred_letter = LETTERS[pred_cls]
                text = f"{idx}. {true_letter} → {pred_letter}: {count}× chybně"
                lbl = tk.Label(
                    list_frame,
                    text=text,
                    font=("Courier", 11),
    ...
    def _on_confusion_click(self, event):
        ...
        true_letter = LETTERS[row]
        pred_letter = LETTERS[col]

        if count == 0:
            messagebox.showinfo(
                "Žádné případy",
                f"Žádné vzorky nebyly klasifikovány jako:\nSkutečnost: {true_letter}, Predikce: {pred_letter}"
            )
            return
    ...
    def _show_confusion_examples(self, true_class: int, pred_class: int, total_count: int):
        ...
        true_letter = LETTERS[true_class]
        pred_letter = LETTERS[pred_class]
        ...
        ex_window.title(f"Příklady: {true_letter} → {pred_letter}")
        ex_window.config(bg=BG_COLOR, padx=20, pady=20)
        ex_window.transient(self.window)

        # Header
        if true_class == pred_class:
            header_text = f"✓ Správně klasifikováno jako {true_letter} ({total_count}× celkem)"
            header_color = "#4CAF50"
        else:
            header_text = f"✗ Skutečnost: {true_letter}, Model řekl: {pred_letter} ({total_count}× celkem)"
            header_color = "#F44336"
    ...
    def _export_charts(self):
        ...
        try:
            ...
            confusion_path = output_path / f"letter_confusion_matrix_{timestamp}.png"
            ...
            # Export metrics
            metrics_path = output_path / f"letter_per_class_metrics_{timestamp}.png"
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Vizualizace výsledků z LetterTester")
    parser.add_argument(
        "--results",
        type=Path,
        default=None,
    ...
    if args.results is None:
        ...
        results_files = sorted(Path(".").glob("letter_test_results_*.json"), reverse=True)
        if not results_files:
            # Try shared/tests
            results_files = sorted(Path("../shared/tests").glob("letter_test_results_*.json"), reverse=True)
        if not results_files:
